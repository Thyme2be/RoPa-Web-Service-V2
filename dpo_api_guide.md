...
| `is_final` | Boolean | **สำคัญ**: ส่งเป็น `true` เมื่อต้องการ "ยืนยันผลการตรวจสอบ" เพื่อเปลี่ยนสถานะเอกสาร หากส่งเป็น `false` จะเป็นการบันทึกร่างเท่านั้น |

**Request Example (บันทึกร่าง)**:
```json
{
  "group": "DO",
  "is_final": false,
  "comments": [
    { "section_key": "DO_SEC_1", "comment": "กำลังตรวจสอบส่วนนี้อยู่..." }
  ]
}
```

**Request Example (ยืนยันการตรวจสอบ)**:
```json
{
  "group": "DO",
  "is_final": true,
...
```
> [!TIP]
> **Logic การอนุมัติ**: หากส่ง `is_final: true` และอาเรย์ `comments` ว่างเปล่า (หรือทุกคอมเมนต์เป็นค่าว่าง) ระบบจะตั้งสถานะเอกสารเป็น `APPROVED` (สมบูรณ์) และอัปเดตวันหมดอายุให้ทันทีครับ
...
**หมายเหตุ**: ระบบจะทำการค้นหาผู้ใช้จาก **คำนำหน้า, ชื่อจริง และนามสกุล** ในฐานข้อมูลโดยอัตโนมัติ หากไม่พบข้อมูลที่ตรงกัน ระบบจะคืนค่า `404 Not Found` พร้อมข้อความแจ้งเตือนให้ลงทะเบียนผู้ใช้ก่อนครับ
...
| `404` | ไม่พบผู้ตรวจสอบรายนี้ (ชื่อ...) ในระบบ | ระบุชื่อผู้ตรวจสอบที่ไม่ตรงกับฐานข้อมูลผู้ใช้งาน |
