...
- `CONFIRMED`: ยีนยันข้อมูลผ่านการคัดกรองแล้ว (รอจัดส่งให้ Owner)
- `SUBMITTED`: กดปุ่มส่งถึง Owner แล้ว (แผงฟอร์มจะถูกล็อก `is_read_only = true`)
- `NEEDS_REVISION`: กรอกข้อมูลไม่ผ่านและถูกตีกลับจากผู้ตรวจสอบ (Revision Requested)
...
**หน้าที่:** จัดส่งเอกสารที่มีสถานะเป็น `CONFIRMED` แล้วเท่านั้น ส่งกลับคืนไปหาหัวหน้า (Owner)
**ผลลัพธ์:** เมื่อจัดส่งแล้ว สถานะฝั่ง DP จะเปลี่ยนเป็น `SUBMITTED` และระบบจะทำการตรวจสอบ "ความสมบูรณ์ทั้งระบบ" ของเอกสารหลัก หากทางฝั่ง DO กรอกเสร็จแล้วด้วย สถานะเอกสารหลักจะเปลี่ยนเป็น `COMPLETED` อัตโนมัติทันทีครับ
