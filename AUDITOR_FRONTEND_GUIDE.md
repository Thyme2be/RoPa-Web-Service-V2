...
|---|---|---|
| `total_documents` | card "จำนวนเอกสารทั้งหมด" | นับ RoPA documents ที่ assigned ให้ auditor นี้ (ใน time_range) — **1 doc = 1 รายการ** (ไม่ใช่ 2 แถว) |
| `pending_review` | card "รายการที่รอตรวจสอบ" | นับ documents ที่ทั้ง owner และ processor **ยังไม่ได้กด "ส่งข้อเสนอแนะ" เลยสักฟอร์ม** |
| `monthly_trend` | กราฟแท่ง (ปีนี้ vs ปีที่แล้ว) | นับ documents ต่อเดือน (ไม่ใช้ time_range filter) แสดงทุกเดือน 1-12 |
...
| `received_at` | คอลัมน์ "วันที่ได้รับ" | วันที่ Data Owner ส่งให้ Auditor |
...
**สิ่งที่เกิดขึ้น (Backend):**
1. Insert บันทึกลงใน `deleted_document_logs` (เก็บ ropa_doc_id, doc_code, title, auditor, deleted_at)
2. `db.delete(doc)` — **ลบถาวรออกจาก DB** (CASCADE ลบ owner_record, processor_records, auditor_audits ด้วย)
