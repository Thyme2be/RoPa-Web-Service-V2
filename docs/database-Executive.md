...
ข้อมูลแผนกมาจาก `users.department` ของ Data Owner ที่สร้างเอกสาร (Admin เป็นคนตั้งตอนสร้าง user)

| Column | Type | Nullable | ใช้ใน Executive | หมายเหตุ |
|--------|------|----------|----------------|---------|
| `id` | `int` | NO | ใช่ — join ผ่าน `ropa_owner_sections.owner_id` | |
| `department` | `varchar` | YES | ใช่ — ชื่อแผนกสำหรับ risk chart และ sensitive chart | Admin ตั้งตอนสร้าง user |
...
### 2. ข้อมูลแผนก (department) มาจาก users.department ของ Data Owner

Executive ดึงแผนกจาก `users.department` ของ Data Owner ที่สร้างเอกสาร (ผ่าน `ropa_owner_sections.owner_id`) สำหรับ:
- `risk_by_department`
...
ข้อมูลแผนกพร้อมใช้ทันทีที่ Admin สร้าง Data Owner และ DO สร้างเอกสาร **ไม่ต้องรอ DPO assign auditor**

> ถ้า Admin สร้าง user โดยไม่ได้ตั้ง department → `users.department = NULL` → เอกสารของ DO คนนั้นจะไม่ปรากฏในกราฟ
...
| ตรวจสอบว่า Admin ตั้ง `users.department` ให้ Data Owner ครบ | Admin / QA | Executive: risk chart และ sensitive chart จะมีข้อมูลครบ |
...
ropa_documents
    ...
    │             └── owner_id → users.department ← แหล่งข้อมูลแผนก
    │
