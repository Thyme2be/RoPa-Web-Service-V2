...
export default function DashboardPage() {
    const { getDashboardStats, refresh, fetchOwnerDashboard, isLoading } = useRopa();
    ...
    useEffect(() => {
        if (timeFilter !== "all" || true) { // Always refresh when filter state changes if we want live updates
                ...
                <div className="p-10 space-y-8">
                    ...
                    {isLoading && (
                        <div className="flex items-center justify-center py-4">
                            ...
                            <span className="ml-3 text-sm font-medium text-muted-foreground italic">กำลังอัปเดตข้อมูล...</span>
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2">
                            <DashboardSummaryCard
                                ...
                                accentColor="danger"
                                splitValues={[
                                    { label: "ผู้รับผิดชอบข้อมูล", value: stats.docsToEdit.owner },
                                    { label: "ผู้ประมวลผลข้อมูลส่วนบุคคล", value: stats.docsToEdit.processor }
                                ]}
                            />
                        </div>
                        ...
                        <div className="lg:col-span-2 space-y-8 flex flex-col h-full">
                            <DashboardSummaryCard
                                ...
                                accentColor="info"
                                splitValues={[
                                    { label: "อยู่ระหว่างตรวจสอบเพื่อจัดเก็บเอกสาร", value: stats.pendingDpo.store },
                                    { label: "อยู่ระหว่างตรวจสอบเพื่อทำลายเอกสาร", value: stats.pendingDpo.destroy }
                                ]}
                            />
                            <DashboardSummaryCard
                                icon="hourglass_top"
                                ...
                                accentColor="info"
                                splitValues={[
                                    { label: "ผู้รับผิดชอบข้อมูล", value: stats.pendingDocs.owner },
                                    { label: "ผู้ประมวลผลข้อมูลส่วนบุคคล", value: stats.pendingDocs.processor }
                                ]}
                            />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-6">
                        <DashboardSummaryCard
                            ...
                            accentColor="info"
                            splitValues={[
                                { label: "เอกสารที่ตรวจสอบแล้วโดยเจ้าหน้าที่คุ้มครองข้อมูลส่วนบุคคล", value: stats.annualCheck.reviewed },
                                { label: "เอกสารที่ยังไม่ได้ตรวจสอบ", value: stats.annualCheck.notReviewed }
                            ]}
                        />
                    </div>
