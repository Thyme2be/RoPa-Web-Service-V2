            ...
            <main className="flex-1 ml-[var(--sidebar-width)] flex flex-col bg-surface-container-low">
                ...
                <div className="p-10 space-y-10">
                    <div className="space-y-1">
                        <h1 className="text-[28px] font-headline font-black text-[#1B1C1C] tracking-tight text-left">
                        ...
                        <table className="w-full text-center border-collapse">
                            <thead>
                                <tr className="border-b border-[#E5E2E1]/40">
                                    <th className="py-5 text-[12.5px] font-black tracking-tight text-[#5C403D] w-[20%] uppercase">รหัสเอกสาร</th>
                                    <th className="py-5 text-[12.5px] font-black tracking-tight text-[#5C403D] w-[40%]">ชื่อรายการ</th>
                                    <th className="py-5 text-[12.5px] font-black tracking-tight text-[#5C403D]">วันที่ส่งข้อมูล</th>
                                    <th className="py-5 text-[12.5px] font-black tracking-tight text-[#5C403D]">สถานะ</th>
                                </tr>
                            </thead>
                    ...
                    <div className="space-y-8">
                        <h2 className="text-[22px] font-headline font-black text-[#5C403D] tracking-tight">
                        ...
                        <div className="bg-white rounded-[24px] shadow-sm border border-[#E5E2E1]/40 border-l-[8px] border-l-[#ED393C] p-10 flex items-center justify-between gap-10">
                            <div className="flex items-center gap-10 flex-1">
                                <label className="text-[17.5px] font-black text-[#1B1C1C] min-w-[140px]">เอกสาร RoPA</label>
                                <div className="flex-1 max-w-2xl relative">
                                    <input
                                        ...
                                        className="w-full h-16 bg-[#F6F3F2] rounded-xl px-7 text-[15px] font-bold text-[#1B1C1C] outline-none border border-transparent focus:bg-white focus:border-[#ED393C]/20 focus:ring-4 focus:ring-[#ED393C]/5 transition-all"
                                    />
                                </div>
                            </div>
                            <button
                                onClick={() => setIsModalOpen(true)}
                                className="bg-[#ED393C] text-white px-10 h-14 rounded-xl font-black text-[15px] shadow-lg shadow-[#ED393C]/20 hover:brightness-110 hover:-translate-y-0.5 transition-all active:scale-95 whitespace-nowrap"
                            >
                                เลือกรายการ
                            </button>
                        </div>
                        ...
                        <div className="flex items-center justify-end gap-4 pt-4">
                            ...
                            <button
                                ...
                                className={`px-10 h-[52px] rounded-xl font-black text-[16px] leading-none transition-all shadow-xl shadow-red-900/20 active:scale-95 bg-logout-gradient text-white ${!selectedDoc ? "opacity-70 cursor-not-allowed" : "hover:brightness-110"
                                    }`}
                            >
                            ...
                            <div className="flex items-center justify-between p-8 border-b border-[#E5E2E1]/40">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-full bg-[#0D9488]/10 flex items-center justify-center">
                                        <span className="material-symbols-rounded text-[#0D9488] text-[24px]">check_circle</span>
                                    </div>
                                    <h3 className="text-[20px] font-headline font-black text-[#1B1C1C] tracking-tight">
                                ...
                                >
                                    <span className="material-symbols-rounded text-[#5F5E5E] text-[24px]">close</span>
                                </button>
                            </div>
                                    ...
                                    <table className="w-full text-center border-collapse">
                                        <thead>
                                            <tr className="border-b border-[#E5E2E1]/20">
                                                <th className="pb-6 text-[13px] font-black text-[#5C403D]">รหัสเอกสาร</th>
                                                <th className="pb-6 text-[13px] font-black text-[#5C403D] text-left px-4">ชื่อรายการ</th>
                                                <th className="pb-6 text-[13px] font-black text-[#5C403D]">วันที่สร้าง</th>
                                                <th className="pb-6 text-[13px] font-black text-[#5C403D]">การดำเนินการ</th>
                                            </tr>
                                        </thead>
                                        <tbody className="divide-y divide-[#E5E2E1]/10">
                                            {paginatedModalRecords.map((doc) => (
                                                <tr key={doc.id} className="hover:bg-gray-50/50 transition-colors group">
                                                    <td className="py-6 text-[14px] font-medium text-secondary">{doc.id}</td>
                                                    <td className="py-6 text-[15px] font-bold text-[#1B1C1C] text-left px-4 group-hover:text-[#ED393C] transition-colors">{doc.documentName}</td>
                                                    <td className="py-6 text-[14px] font-medium text-secondary">{doc.dateCreated}</td>
                                                    <td className="py-6">
                                                        <button
                                                            onClick={() => handleSelect(doc)}
                                                            className="bg-[#E5E2E1] text-[#1B1C1C] px-6 py-2.5 rounded-lg font-black text-[13px] hover:bg-[#D6D3D1] transition-all shadow-sm active:scale-95"
                                                        >
                                                            เลือกเอกสาร
                                                        </button>
                                                    </td>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        {[...Array(totalModalPages)].map((_, i) => (
                                            <button
                                                ...
                                                className={`w-9 h-9 rounded-lg text-[14px] font-bold transition-all ${modalPage === i + 1
...
function ReviewRow({ id, title, date, status }: any) {
    return (
        <tr className="hover:bg-gray-50/50 transition-all group border-b border-[#E5E2E1]/20">
            <td className="py-7 text-[13.5px] font-medium text-secondary uppercase">{id}</td>
            <td className="py-7 text-[15.5px] font-extrabold text-[#1B1C1C] tracking-tight leading-snug group-hover:text-[#ED393C] transition-colors">{title}</td>
            <td className="py-7 text-[13.5px] font-medium text-secondary">{date}</td>
