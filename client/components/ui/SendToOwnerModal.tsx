...
interface SendToOwnerModalProps {
    isOpen: boolean;
    onClose: () => void;
    onConfirm: () => void;
    isLoading?: boolean;
...
export default function SendToOwnerModal({
    isOpen,
    onClose,
    onConfirm,
    isLoading = false
}: SendToOwnerModalProps) {
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-6 animate-in fade-in duration-300">
            ...
            <div className="bg-white w-full max-w-[540px] rounded-[48px] shadow-2xl relative p-14 flex flex-col items-center text-center animate-in zoom-in-95 duration-300">
                ...
                <div className="space-y-3 mb-12 mt-6">
                    <h2 className="text-[36px] font-black text-[#1B1C1C] tracking-tight leading-tight">
                        ยืนยันการส่งเอกสาร
                    </h2>
                    <p className="text-[17px] font-bold text-[#5F5E5E] tracking-tight px-4">
                        ตรวจสอบข้อมูลในเอกสารก่อนส่งให้ผู้รับผิดชอบข้อมูล
                    </p>
                </div>
